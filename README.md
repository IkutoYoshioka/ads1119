UI層（CamelCase）
-------------------------
API / Schema 層（CamelCase 外向き）
-------------------------
CRUD 層（Camel → Snake 変換）
-------------------------
DB / Model 層（SnakeCase）
-------------------------

### API設計

- API設計の第一歩は、そのAPIを使ってユーザーが何を達成できるのかを見極めること。
...
**誰が**　APIのユーザーをリストアップする  
**何を**　それらのユーザーが何をできるかリストアップする  
**どのように**　「何を」のそれぞれを複数ステップに分解する  
**入力（ソース）**　各ステップに必要なものと、それがどこにあるのかリストアップする  
**出力（用途）**　各ステップで返されるものと、それをどのように使うのかリストアップする  
**ゴール**　「どのように」＋「入力」＋「出力」をわかりやすい表現に言い換える
